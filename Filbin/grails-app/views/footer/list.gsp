...
<section id="list-footer" class="first">

	<table class="table table-bordered margin-top-medium">
		<thead>
			<tr>
			
				<g:sortableColumn property="title" title="${message(code: 'footer.title.label', default: 'Title')}" />
		...
		<g:each in="${footerInstanceList}" status="i" var="footerInstance">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td><g:link action="show" id="${footerInstance.id}">${fieldValue(bean: footerInstance, field: "title")}</g:link></td>
